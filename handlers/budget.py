...
class Budget(StatesGroup):
    budget_name_expenses = State()
    option = State()
    budget_period_expenses = State()
    budget_amount_expenses = State()
    add_budget_name_expenses = State()
    add_budget_amount_expenses = State()

@budget_router.message(StateFilter('*'), F.text.contains('Добавить бюджет'))
async def budget(message: Message, session: AsyncSession, state: FSMContext):
    ...
    if cat:
        ...
        await state.set_state(Budget.add_budget_name_expenses)
    else:
        text_profile = (
...
@budget_router.message(F.text == 'Бюджет 🧾')
async def budget(message: Message, session: AsyncSession, state: FSMContext):
    user_id = message.from_user.id
    ...
    await state.set_state(Budget.budget_period_expenses)

@budget_router.message(Budget.budget_period_expenses, lambda c: c.text in ['День','Месяц', 'Год'])
async def budget(message: Message, session: AsyncSession, state: FSMContext):
    ...
    await state.set_state(Budget.budget_name_expenses)


@budget_router.callback_query(Budget.budget_name_expenses, F.data)
async def profile_1(callback: types.CallbackQuery, session: AsyncSession,state: FSMContext):
    ...
    await state.set_state(Budget.option)

@budget_router.callback_query(Budget.option, F.data)
async def profile_1(callback: types.CallbackQuery, session: AsyncSession,state: FSMContext):
    ...
    await state.set_state(Budget.budget_amount_expenses)

@budget_router.message(Budget.budget_amount_expenses, F.text)
...
@budget_router.callback_query(Budget.add_budget_name_expenses, F.data)
async def budget(callback: types.CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    await state.set_state(Budget.add_budget_amount_expenses)


@budget_router.message(Budget.add_budget_amount_expenses, F.text)
async def budget(message: Message, session: AsyncSession, state: FSMContext):
